<div class="big-content">
    ...
    <div class="big">
        @if(count($microposts) > 0)
                    ...
                    <div class="big__freme__container">
                        ...
                        @if(Auth::id() != $micropost->user_id)
                            @if (Auth::user()->is_favorites($micropost->id))
                                {{-- お気に入り解除のボタンのフォーム --}}
                                {!! Form::open(['route' => ['favorites.unfavorite', $micropost->id], 'method' => 'delete','class' => "big__freme__container-form"]) !!}
         ...
         @else
            {{-- お気に入り追加のボタンのフォーム --}}
            {!! Form::open(['route' => ['favorites.favorite', $micropost->id], 'class' => "big__freme__container-form"]) !!}
                {{--{!! Form::submit('お気に入り', ['class' => "btn btn-secondary "]) !!}--}}
                {!! Form::button('<i class="fas fa-heart favorite big__freme__container-btn"></i>', ['class' => "btn btn-favorite", 'type' => 'submit']) !!}
            {!! Form::close() !!}
